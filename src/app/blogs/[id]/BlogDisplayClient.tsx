...
const BlogDisplayClient: React.FC<BlogDisplayClientProps> = ({ blogMetadata }) => {
  ...
  useEffect(() => {
    if (!blogMetadata || !blogMetadata.id || !blogMetadata.contentStoragePath || !blogMetadata.restaurantId) {
      if (blogMetadata) { 
        setContentError("Required information to fetch blog content is missing.");
      }
    ...
    const fetchContentFromStorage = async () => {
      ...
      try {
        const pathReference = storageRef(storage, blogMetadata.contentStoragePath);
        ...
        if (contentString) {
          ...
          try {
            ...
            setContentError("Blog content is malformed and could not be displayed.");
        ...
        setContentError(err instanceof Error ? `Failed to load content: ${err.message}` : 'An unknown error occurred while fetching content.');
        setBlogContent({ type: "doc", content: [] }); // Fallback
      } finally {
        setContentLoading(false);
      }
  ...
  const editor = useEditor({
    ...
    extensions: [
      ...
      TiptapImageExtension.configure({ inline: false, allowBase64: true }),
      ...
      Subscript, Superscript, Underline, TaskList, TaskItem.configure({ nested: true }),
    ],
    editorProps: {
      attributes: {
  ...
  useEffect(() => {
    ...
    if (!container || typeof videoUrlFromProps !== 'string' || !videoUrlFromProps.startsWith('http')) {
        if (videoUrlFromProps) console.warn("Invalid videoUrlFromProps for IntersectionObserver:", videoUrlFromProps);
        return;
    }
    const observer = new IntersectionObserver(([entry]) => setIsMainVideoVisible(entry.isIntersecting), { threshold: 0.1 });
  ...
  useEffect(() => {
    ...
    if (!video || typeof videoUrlFromProps !== 'string' || !videoUrlFromProps.startsWith('http')) {
        return;
    }

    const handlePlay = () => {
        // User/autoplay initiated play on main video
        if (!isVideoPoppedOut) { // Only update if main video is the active one
  ...
  useEffect(() => {
    if (typeof videoUrlFromProps === 'string' && videoUrlFromProps.startsWith('http') &&
        !isMainVideoVisible && isMainVideoPlaying) {
  ...
  useEffect(() => {
    ...
    if (!mainVideo || !popOut || typeof videoUrlFromProps !== 'string' || !videoUrlFromProps.startsWith('http')) {
        return;
    }
    
    const playVideoAsync = async (videoElement: HTMLVideoElement, isPlayingMain: boolean) => {
        try {
            await videoElement.play();
            if (isPlayingMain && videoElement === mainVideo && !videoElement.paused) {
            ...
            if (domError.name !== 'AbortError') {
                console.warn(`${videoElement === mainVideo ? 'Main' : 'Pop-out'} video play() failed:`, domError);
            }
        }
    };
  ...
  }, [isVideoPoppedOut, videoUrlFromProps, isMainVideoVisible, isMainVideoPlaying]);
  ...
  if (!blogMetadata) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen text-primary">
        ...
        <button onClick={() => router.push('/blogs')} className="mt-4 px-6 py-2 bg-primary text-white font-semibold rounded-md hover:bg-primary-dark transition-colors">
          Back to Blogs
        </button>
      </div>
    );
    ...
    <div className="font-creto overflow-y-auto min-h-screen">
      <article className="bg-white shadow-xl rounded-lg">
        <div className='flex flex-col sm:flex-row px-8 pt-4 sm:pt-6 md:pt-8 max-w-4xl mx-auto'>
          ...
          <div className={`p-4 sm:p-6 md:p-8 flex flex-col justify-center items-start ${blogMetadata.coverImage ? 'sm:ml-6' : 'w-full'}`}> 
            ...
            <p className="text-sm text-gray-500 mb-6 border-b pb-4">
              ...
              {updatedAtDate && blogMetadata.updatedAt && blogMetadata.createdAt &&
        ...
        {typeof videoUrlFromProps === 'string' && videoUrlFromProps.startsWith('http') && (
          <div ref={videoContainerRef} className="my-6 max-w-3xl mx-auto px-4 sm:px-0">
            <video
              ref={mainVideoRef}
        ...
        <div className={`p-6 md:p-10 sm:px-20 md:max-w-3xl transition-all duration-500 ease-in-out ${isVideoPoppedOut ? 'md:ml-[286px] mx-auto' : 'mx-auto'}`}>
          <div className="tiptap-rendered-content ">
            ...
            {contentError && !contentLoading && (
                 <div className="p-4 my-4 text-sm text-red-700 bg-red-100 rounded-lg" role="alert">
                    <span className="font-medium">Content Error:</span> {contentError}
                 </div>
            )}
          </div>
        </div>
