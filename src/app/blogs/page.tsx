      ...
      try {
        ...
        if (direction === "initial") {
          ...
          if (lastVisible) {
            ...
            setError("Cannot fetch next page: no reference point (lastVisible is null).");
            setLoading(false);
            return;
          }
          ...
          if (firstVisible) {
            ...
            setError("Cannot fetch previous page: no reference point (firstVisible is null).");
            setLoading(false);
            return;
          }
        }
        ...
        if (!q) {
          ...
          console.error(`Query construction failed for direction: ${direction}. This indicates an unexpected state.`);
          setError("An internal error occurred while preparing to fetch blogs. Invalid direction or state.");
          setLoading(false);
          return;
        }
        ...
        if (querySnapshot.docs.length > 0) {
          ...
          if (direction === "prev") setCurrentPage((prev) => Math.max(1, prev - 1)); // Ensure page doesn't go below 1
        } else {
          if (direction === "initial") {
            setBlogs([]);
          }
  ...
  if (error && blogs.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen p-4">
        ...
        {!restaurantId && <p className="text-sm text-gray-500 mt-2">Hint: Restaurant ID might be missing in environment variables.</p>}
      </div>
    );
  }
      ...
      {!loading && blogs.length === 0 ? (
        <div className="text-center py-12">
          ...
          {process.env.NODE_ENV === 'development' && restaurantId && (
            <p className="text-sm text-gray-400 mt-1">Checked path: Restaurants/{restaurantId}/blogs</p>
          )}
           {!restaurantId && process.env.NODE_ENV === 'development' && (
            <p className="text-sm text-orange-500 mt-1">Warning: Restaurant ID (NEXT_PUBLIC_FIREBASE_RESTAURANT_ID) is not set.</p>
          )}
        </div>
      ) : !loading && blogs.length > 0 ? (
        <>
          ...
          {viewMode === "grid" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {blogs.map((blog, index) => (
                (!blog.archived || user != null) && (blog.id != "HhrqYszvCsMXXo2gywSO") &&
                <Link key={blog.id} href={`/blogs/${blog.id}`} legacyBehavior>
                  <a className={commonLinkClasses}>
                    ...
                    <div className="p-6">
                      ...
                      <div className="mt-4 flex justify-between">
                        ...
                        {user &&
                        <div className="flex items-center gap-2">
                          {blog.archived && <div className=" h-fit p-1 text-primary text-normal4 rounded bg-primary/20 border border-primary">archieved</div>}
