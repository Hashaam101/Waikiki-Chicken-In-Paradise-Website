...
// import body from '@/../public/Svgs/bird/body.svg';
// import head from '@/../public/Svgs/bird/head.svg';
...
// const SubscriptionPopup: React.FC<SubscriptionPopupProps> = ({ isOpen, setIsOpen, config }) => {
...
//             const subscribersCollectionRef = collection(db, `Restaurants/${process.env.NEXT_PUBLIC_FIREBASE_RESTAURANT_ID}/Subscribers`);
...
//                 <div className="-z-10 absolute top-0 left-0 h-full w-1/2" style={{ backgroundImage: `url(${bg.src})` }}></div>
//                 <div className="-z-10 absolute top-0 right-0 rotate-y-180 h-full w-1/2" style={{ backgroundImage: `url(${bg.src})` }}></div>
...
//                                 {config.mode === 'redirect' ? 'Redirecting you now...' : "You're subscribed!"}
//                             </p>
//                         </div>
//                     ) : (
//                         // Form State - Use config for text
//                         <>
//                             <h2 className="text-primary-dark text-h5 font-semibold mb-3 text-center font-not-east">{config.title}</h2>
//                             <p className="text-grey mb-6 text-center text-normal4 sm:text-normal3">{config.message}</p>
...
//                                                 src={head}
//                                                 alt="Background"
//                                                 className="absolute top-0 left-0 w-fit h-[29px] -translate-x-[15px] -translate-y-[22px] group-hover:-rotate-10 group-hover:-translate-y-[20px] transition-transform duration-300 ease-in-out"
//                                                 priority
//                                             />
//                                             <Image
//                                                 src={body}
//                                                 alt="Background"
//                                                 className="absolute top-0 left-0 w-fit h-[69px] -translate-x-[46px] -translate-y-[7px]"
//                                                 priority
//                                             />
//                                         </div>
//                                         <button
//                                             type="submit"
//                                             disabled={isLoading}
//                                             className={`group w-fit px-5 py-2.5 rounded-lg text-white font-medium text-normal2 transition-colors duration-200 ease-in-out flex items-center justify-center ${isLoading
